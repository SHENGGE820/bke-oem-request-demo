(function(){
  ...
  function update(item){const cases=rawAll(),index=cases.findIndex(entry=>entry.id===item.id),remaining=incompleteTasks(item);item.status=remaining.length?'進行中':'已完成';item.updatedAt=now();if(index>=0)cases[index]=item;else cases.unshift(item);save(cases);syncPageHandoff(item);return item}
