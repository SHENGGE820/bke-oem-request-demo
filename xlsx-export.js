...
(function(){
  ...
  function buildA(c){
    ...
    const 劑型=txt(s['目標劑型']);
    const unitOf={'目標成本':' 元／單位','產品售價':' 元'};
    const withUnit=key=>{const v=txt(s[key]);return v?(/^[0-9.]+$/.test(v)?v+(unitOf[key]||''):v):'';};
    const 粉劑spec=txt(s['粉劑規格'])?txt(s['粉劑規格'])+'g／包':'';
    const 膠囊Spec=[txt(s['膠囊材質']),txt(s['膠囊號數'])].filter(Boolean).join(' ');
    const 錠劑Spec=[txt(s['錠劑形式']),txt(s['錠劑重量'])?txt(s['錠劑重量'])+'mg／顆':''].filter(Boolean).join(' ');
    const 液態spec=txt(s['液態規格'])?txt(s['液態規格'])+'ml／份':'';
    const 果凍spec=txt(s['果凍規格'])?txt(s['果凍規格'])+'g／份':'';
    const 族群年齡=txt(s['族群年齡']);
    const seg=[
      {c:'B',r:1,t:'申請人員',s:'L'},{c:'C',r:1,t:txt(s['申請人員']||c.applicant),s:'V'},{c:'D',r:1,t:'申請日期',s:'L'},{c:'E',r:1,t:txt(s['申請日期']||(c.createdAt?new Date(c.createdAt).toLocaleDateString('sv-SE'):'')),s:'V'},{c:'F',r:1,t:'單號',s:'L'},{c:'G',r:1,t:txt(s['單號']||c.id),s:'V'},
      ...
      {c:'B',r:4,t:'目標族群',s:'L'},{c:'C',r:4,t:box('全族群',has(s['族群'],'全族群'))+'  '+box('學童',has(s['族群'],'學童')),s:'O'},{c:'D',c2:'E',r:4,t:box('男性',has(s['族群'],'男性'))+'  '+box('女性',has(s['族群'],'女性'))+'  '+box('銀髮',has(s['族群'],'銀髮')),s:'O'},{c:'F',c2:'G',r:4,t:box('特殊族群',has(s['族群'],'特殊'),txt(s['特殊族群說明']))+(族群年齡?('\n年齡：'+族群年齡):''),s:'O'},
      {c:'B',r:5,t:'主訴調理功能',s:'L'},{c:'C',c2:'G',r:5,t:'調理部位：'+txt(s['調理部位'])+(s['其他調理部位']?('（'+txt(s['其他調理部位'])+'）'):'')+'\n期望調理功能：'+txt(s['期望功能'])+(s['其他期望方向']?('（'+txt(s['其他期望方向'])+'）'):'')+'\n補充說明：'+txt(s['客戶需求']),s:'O'},
      {c:'B',r:6,t:'目標成本/產品售價',s:'L'},{c:'C',c2:'G',r:6,t:'目標成本：'+withUnit('目標成本')+'　　產品售價：'+withUnit('產品售價')+'\n（配方原料成本不含代工包裝）',s:'O'},
      {c:'B',r:7,t:'目標劑型',s:'L'},{c:'C',c2:'G',r:7,t:[box('粉劑',/粉/.test(劑型),粉劑spec),box('膠囊',/膠囊/.test(劑型),膠囊Spec),box('錠劑',/錠/.test(劑型),錠劑Spec),box('液態',/液/.test(劑型),液態spec),box('果凍',/果凍/.test(劑型),果凍spec),box('其他特殊劑型',/其他/.test(劑型),txt(s['其他劑型說明']))].join('  '),s:'O'},
